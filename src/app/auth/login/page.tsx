    ...
    <main className="min-h-screen bg-white flex items-center justify-center">
      <div className="w-full max-w-sm px-4">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-500 text-sm">{t('auth.signInTitle')}</p>
        </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-500">
          {t('auth.noAccount')}{' '}
          <Link href={`/auth/signup${redirect !== '/' ? `?redirect=${encodeURIComponent(redirect)}` : ''}`} className="text-blue-600 hover:underline font-medium">
            {t('common.signUp')}
          </Link>
        </p>
      </div>
